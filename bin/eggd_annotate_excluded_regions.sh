...
set -exo pipefail
panel_bed="$1"
additional_regions="$2"
excluded_regions="$3"
cds_hgnc="$4"
cds_gene="$5"
ouput_prefix="$6"
excluded_regions_prefix="$(basename -- $excluded_regions .bed)"
main() {
    ...
    if [ "$excluded_regions" ] ; then
        ...
        cut -f 1,2,3 "$excluded_regions" >  ${excluded_regions_prefix}_nf.bed
        excluded_regions=${excluded_regions_prefix}_nf.bed
        ...
        if [ "$additional_regions" ]; then
            ...
            sed 1d $additional_regions | cat $cds_hgnc - | sort -k1,1 -k2,2n > cds_exons.tsv
        else
            echo "No additional regions file is provided"
            mv $cds_hgnc cds_exons.tsv
        fi
        ...
        if [ -z "$panel_bed" ]; then
            ...
            nextflow-bin/bedtools intersect -b cds_exons.tsv -a $excluded_regions -wao > excluded_genes.bed
            python3 nextflow-bin/annotate_excluded_panel.py -e excluded_genes.bed -r $excluded_regions -c $cds_gene
            ...
            nextflow-bin/bedtools intersect -a $excluded_regions -b $panel_bed -wa | sort | uniq > panel_excluded.bed
            ...
            if [ -s panel_excluded.bed ]; then
                ...
                nextflow-bin/bedtools intersect -b cds_exons.tsv -a panel_excluded.bed -wao > panel_excluded_genes.bed
                python3 nextflow-bin/annotate_excluded_panel.py -e panel_excluded_genes.bed -p $panel_bed -r $excluded_regions -c $cds_gene
            else
                echo "Panel regions do not overlap with cnv calling excluded regions."
                printf "Chrom\tStart\tEnd\tLength\tGene_Symbol\tHGNC_ID\tTranscript\tExon\n" | tee touch annotated_$(echo ${excluded_regions##*/} | sed 's/.bed//g')_${panel_bed##*/}
            fi
        fi
    else
    ...
    if [ "$linecount" -gt 1 ]; then
        ...
        if [ "$header" ]; then
            ...
            rm $out_file;
            mv ${out_file%.*}.tsv ${ouput_prefix}-${out_file%.*}.tsv
        else
            echo "Incorrect output header"
        fi
    else
        echo "Empty file with headers only"
        mv $out_file ${ouput_prefix}-${out_file%.*}.tsv;
    fi
